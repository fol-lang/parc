...
struct TracedToken {
    ...
    /// An empty marker, not a token: `#pragma pack` turning packing on
    /// (`Some(true)`) or off, placed where it took effect in the output.
    pack_mark: Option<bool>,
}
...
pub(super) struct TracedPreprocessed {
    ...
    /// Generated byte ranges written while `#pragma pack` was active.
    pub packed_ranges: Vec<(usize, usize)>,
...
struct TracedProcessor<'a> {
    ...
    /// `#pragma pack`: whether packing is active, and the pushed states.
    packing: bool,
    packing_stack: Vec<bool>,
...
pub(super) fn preprocess_builtin_traced(
    ...
    for token in output {
        ...
        match (token.pack_mark, packed_since) {
            (Some(true), None) => {
                packed_since = Some(text.len());
                continue;
            }
            (Some(false), Some(start)) => {
                packed_ranges.push((start, text.len()));
                packed_since = None;
                continue;
            }
            (Some(_), _) => continue,
    ...
    if let Some(start) = packed_since {
        packed_ranges.push((start, text.len()));
    }
...
impl TracedProcessor<'_> {
                ...
                match directive {
                    ...
                    Directive::Pragma { tokens } => {
                        ...
                        if spelling.trim() == "once" {
                            ...
                            // else: those are marked and refused on their own,
                            // and a routine that never reaches one is not.
                            // An unrecognised spelling still refuses the whole
    ...
    fn apply_pack_pragma(&mut self, spelling: &str) -> bool {
        ...
        let is_width = |value: &str| matches!(value, "1" | "2" | "4" | "8" | "16");
        match arguments.split(',').collect::<Vec<_>>().as_slice() {
            [""] => self.packing = false,
            ["push"] => self.packing_stack.push(self.packing),
            ["push", width] if is_width(width) => {
                self.packing_stack.push(self.packing);
                self.packing = true;
            }
            ["pop"] => self.packing = self.packing_stack.pop().unwrap_or(false),
            [width] if is_width(width) => self.packing = true,
            _ => return false,
        }
        true
    }

...
struct TracedToken {
    ...
    /// An empty marker, not a token: `#pragma pack` setting the packing width
    /// in bytes (`Some(Some(n))`) or turning it off, placed where it took effect.
    pack_mark: Option<Option<u64>>,
}
...
pub(super) struct TracedPreprocessed {
    ...
    /// Generated byte ranges written while `#pragma pack` was active, each with
    /// the packing width in bytes it imposed.
    pub packed_ranges: Vec<(usize, usize, u64)>,
...
struct TracedProcessor<'a> {
    ...
    /// `#pragma pack`: the active packing width in bytes, and the pushed states.
    packing: Option<u64>,
    packing_stack: Vec<Option<u64>>,
...
pub(super) fn preprocess_builtin_traced(
    ...
    let mut packed_since = None;
    for token in output {
        if token.kind == TokenKind::Eof || processor.stopped {
            continue;
        }
        if let Some(width) = token.pack_mark {
            if let Some((start, open)) = packed_since.take() {
                packed_ranges.push((start, text.len(), open));
            }
            packed_since = width.map(|width| (text.len(), width));
            continue;
        }
    ...
    if let Some((start, width)) = packed_since {
        packed_ranges.push((start, text.len(), width));
    }
...
impl TracedProcessor<'_> {
                ...
                match directive {
                    ...
                    Directive::Pragma { tokens } => {
                        ...
                        if spelling.trim() == "once" {
                            ...
                            // A recognised `pack` changes the layout of the
                            // records defined while it is active, and nothing
                            // else: each of those carries its width. An
                            // unrecognised spelling still refuses the whole
                            // package, because its reach is unknown.
                            let before = self.packing;
                            let recognised = self.apply_pack_pragma(&spelling);
                            self.issues.push(TraceIssue {
                                code: "PARC-E2104",
                                severity: if recognised {
                                    Severity::Warning
                                } else {
                                    Severity::Error
                                },
                                impact: if recognised {
                                    DiagnosticCompletenessImpact::Informational
                                } else {
                                    DiagnosticCompletenessImpact::ForcesRejected
                                },
                                message: if recognised {
                                    "#pragma pack sets the packing width of the records it spans"
                                } else {
                                    "#pragma pack form is ABI-relevant and not modeled"
                                }
                                .to_owned(),
                                range: Some(range),
                            });
                            if recognised && self.packing != before {
    ...
    /// Apply one `#pragma pack` to the packing state, or report that its form
    /// is not one this models: `pack(push[, N])`, `pack(pop)`, `pack(N)`, and
    /// `pack()`.
    fn apply_pack_pragma(&mut self, spelling: &str) -> bool {
        ...
        let width = |value: &str| match value {
            "1" | "2" | "4" | "8" | "16" => value.parse::<u64>().ok(),
            _ => None,
        };
        match arguments.split(',').collect::<Vec<_>>().as_slice() {
            [""] => self.packing = None,
            ["push"] => self.packing_stack.push(self.packing),
            ["push", bytes] if width(bytes).is_some() => {
                self.packing_stack.push(self.packing);
                self.packing = width(bytes);
            }
            ["pop"] => self.packing = self.packing_stack.pop().flatten(),
            [bytes] if width(bytes).is_some() => self.packing = width(bytes),
            _ => return false,
        }
        true
    }
